...
def trigger_dag(data, context=None):
    ...
    client_id = '785228735105-7v1vk36d7k0uij2uo3gahu6oi10ldu32.apps.googleusercontent.com'
    
    # This should be part of your webserver's URL: https://{tenant-project-id}.appspot.com
    webserver_id = 'lc671f9a53c03e59cp-tp'
    
    # The name of the DAG you wish to trigger
    dag_name = 'model_training_dag'
    # dag_name = 'model_prediction_dag'
    ...
    print(f'webserver_url={webserver_url}, client_id={client_id}, method=POST, json_data={json_data}')
...
def make_iap_request(url, client_id, method='GET', **kwargs):
    ...
    resp = requests.request(method=method, url=url, headers={'Authorization': 'Bearer {}'.format(google_open_id_connect_token)}, **kwargs)
    try:
        if resp.status_code == 403:
            raise Exception('Service account does not have permission to access the IAP-protected application.')
        elif resp.status_code != 200:
            raise Exception(f'Bad response from application:\nResponse.status_code{resp.status_code}\nResponse.Headers={resp.headers}\nResponse.Text={resp.text}')
    except Exception as e:
        print(f'Something went wrong with your POST request: {e}')
    return resp.text
# END COPIED IAP CODE